...
if (token_signature === "LunarRecNew_ReplaceMeWithJustRandomLetters") { //for example "UDDTSGTART"
...
if (allow2016AndEarly2017) {
	...
	log(LogType.Warn, "This server has \"allow2016AndEarly2017\" in config set to true, this function is not made completly, it can sometimes crash your server!")
...
console.log(`${" ".repeat((versionStr.length-"lunarrec".length)/2)}${chalk.hex(colors.logo)("LunarRecNew")}\n${versionStr}\n${"=".repeat(versionStr.length+1)}`)
...
It looks like this is your first time running LunarRecNew.
Please check out the github repo's wiki for first time install steps.
Also note that LunarRecNew is still a heavy work in progress project that is not production ready.
